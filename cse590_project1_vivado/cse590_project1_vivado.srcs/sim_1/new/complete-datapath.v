...
//    reg clock, start;
    ...
    instruction_mem im_inst(.pc_out(pc_out),.clock(clock),.instruction(instruction));
    ...
    jump_adder j_add_inst(.address(address),.pc_out(pc_out),.jump_out(jump_out));
    
    pc_adder pc_add_inst(.se_immediate(se_immediate),.pc_out(pc_out),.branch_out(branch_out));
    
    branch_selector bs_inst(.beq(beq),.blt(blt),.zero_signal(zero_signal),.branch_selection(branch_selection));
    
    mux_2_to_1_16bit mux_inst_3(.A(pc_out),.B(branch_out),.src(branch_selection),.out(muxed_branch_out));
